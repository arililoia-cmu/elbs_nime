...
//     o2ws_clock_msgs_enable (initial value is true) can be set false
//         to suppress printing raw cs/put and cs/get messages.
...
var o2ws_clock_msgs_enable = true;
// clock sync:
...
var o2ws_min_rtt = 999;  // most recent minimum round-trip time used 
var o2ws_max_rtt = 999;  // maximum round-trip time in last 5 pings
...
function o2ws_pending_send() {
    ...
    while (o2ws_msg_out_queue.length != 0  &&
        ...
        if (o2ws_status_msgs_enable &&
            (o2ws_clock_msgs_enable || !msg.startsWith("!_o2/ws/cs/get"))) {
...
function o2ws_csput_handler(timestamp, address, typespec, info) {
    ...
    if (o2ws_ping_reply_count >= CLOCK_SYNC_HISTORY_LEN) {
        ...
        if (!o2ws_clock_synchronized) { // set global clock to our best estimate
            ...
            o2ws_status_msg("Clock sync obtained. Time is " + o2ws_time_get());
        ...
        // save stats for reporting to server
        o2ws_min_rtt = min_rtt;
        o2ws_max_rtt = max_rtt;
...
// helper function - return the mean rtt. 
function get_o2ws_min_rtt(){
    return o2ws_min_rtt;
}


// this gets called when it's time for a clock ping. The first call is
// at o2ws_start_sync_time. After that, pings are sent every 0.1s for 
// the first second, then every 0.5s for 4 seconds, then every 10s.
function o2ws_clock_callback(id) {
    ...
    o2ws_clock_sync_id++;
    o2ws_send_start("!_o2/ws/cs/get", 0, "is", false);
...
function o2ws_initialize(ensemble) {
    if (!o2ws_websocket) {
        ...
        o2ws_websocket.onmessage = function(evt) { 
            o2ws_message_handler(evt);
...
function o2ws_message_handler(evt) {
    ...
    if (o2ws_status_msgs_enable &&
        (o2ws_clock_msgs_enable ||  // maybe filter out cs/get, cs/put
         (address != "_o2/cs/put" && address != "_o2/ws/cs/get"))) {
    ...
    if (o2ws_method_dict.hasOwnProperty(address)) {
        ...
        // o2ws_status_msg("  FIELDS: " + o2ws_message_fields);
...
var o2ws_local_time = o2ws_time_from_date;
...
// DEBUG: strangely, this seemed to return the same exact value
// many times creating a loop where time did not advance.
// If time does not advance by 1ms in 100 calls, cause an
// exception.
var dbg_save_time = 0;
var dbg_time_count = 0;

function o2ws_time_get() {
    if (o2ws_clock_synchronized) {
        let t = o2ws_local_time() + o2ws_global_minus_local;
        if (t > dbg_save_time) {
            dbg_save_time = t;
            dbg_time_count = 0;
        } else {
            dbg_time_count += 1;
            if (dbg_time_count > 100) {
                throw new Error("o2ws_time_get() is not advancing");
            }
        }
        return t;
    } else {
...
function o2ws_get_string() {
    // o2ws_status_msg("    o2ws_get_string from " + o2ws_message_fields);
